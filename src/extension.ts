...
var activeScheme: Record<string, string>;
...
export async function activate(context: vscode.ExtensionContext) {
	...
	const setAllCommand = vscode.commands.registerCommand('flutter-color-scheme-picker.setAll', async () => {
		...
		if (!dynamicSchemeVariant) return;
	...
	const csProvider = vscode.languages.registerCompletionItemProvider(
			...
			provideCompletionItems(
				...
				return Object.entries(activeScheme).map(([name, hex]) => {
					...
					item.insertText = new vscode.SnippetString(
						`Theme.of(context).colorScheme.${name}`
					);
	...
	context.subscriptions.push(
		...
		setDynamicSchemeVariantCommand,
		setContrastLevel,
		csProvider
	);
...
async function generateColorScheme() {
	const config = vscode.workspace.getConfiguration('flutterTheme');
	const seedColor = config.get('seedColor') as string;
	const brightness = config.get('brightness') as string;
	const dynamicSchemeVariant = config.get('dynamicSchemeVariant') as keyof typeof schemeMap;
	const contrastLevel = config.get('contrastLevel') as number;
...
function extractColorHexes(scheme: DynamicScheme): Record<string, string> {
...
function showParamsOnOutput(): void {
	const config = vscode.workspace.getConfiguration('flutterTheme');
...
async function writeSnippets(scheme: Record<string, string>) {
